...
df18ocdb_272_20 <- data2018 %>% 
  mutate(harvest = lagoon_272_10 + ocdb_272_20, # This is where you change things to look at different harvest data sets. 
         run = esc + harvest, #catch is our estimate of what would have occured at the wier had there been no fishing, based on migration timing studies.
         run_early_gen = prop_early_genetics*run)

df18ocdb_272_30 <- data2018 %>% 
  mutate(harvest = lagoon_272_10 + ocdb_272_20 + ocdb_272_30, # This is where you change things to look at different harvest data sets. 
         run = esc + harvest, #catch is our estimate of what would have occured at the wier had there been no fishing, based on migration timing studies.
         run_early_gen = prop_early_genetics*run)

#Cannot seem get this to work.  
define_harvest() <- function(df = data2018, start_area = lagoon_272_10, end_area = lagoon_272_10){
  df <- df %>% 
    mutate(harvest = sum(df$start_area:df$end_area), # This is where you change things to look at different harvest data sets. 
          run = esc + harvest, #catch is our estimate of what would have occured at the wier had there been no fishing, based on migration timing studies.
          run_early_gen = prop_early_genetics*run)
  return(df)
}
