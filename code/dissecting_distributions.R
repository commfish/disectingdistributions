...
read_csv('data/ChigISGrunappt2006-2017.csv') %>% 
  ...
  mutate(esc = early_esc_genetics + late_esc_genetics,
         ...
         run_early = prop_early_genetics*run,
         date = mdy(date),
...
(fit <- mix(as.mixdata(weir_2006), mixparam(mu=c(13335, 13355, 13380), sigma= c(10,10,10)),constr = mixconstr(consigma="SEQ"), dist= 'gamma', iterlim=5000)) 
(fit <- mix(as.mixdata(weir_2006), mixparam(mu=c(13335, 13355, 13380), sigma= c(10,10,10)),constr = mixconstr(conmu="MES"), dist= 'gamma', iterlim=5000))
(fit <- mix(as.mixdata(weir_2006), mixparam(mu=c(13335, 13355, 13380), sigma= c(10,10,10)),dist= 'gamma', iterlim=5000)) 
...
percent_dist(fit, 13338)
percent_dist(fit, 1333)
...
(fit <- mix(as.mixdata(weir_2007), mixparam(mu=c(13687, 13712,13749), sigma= c(10,8, 8)),constr = mixconstr(consigma="SEQ"), dist= 'gamma', iterlim=5000)) 


dist_plot(fit, 2007)
percent_dist(fit, 13697) #date when fifty percent is early run. According to distributions? 
dnormfit(fit, 13697)
percent_dist(fit, 13702)

#2008
weir_2008 <- data_prep(weir_data, 2008)
graph_year(weir_2008)
fit <- distribution_estimation_norms(weir_2008) 
fit <- distribution_estimation(weir_2008, 3, mean_guess_given = c(14050, 14080, 14110 )) 
(fit <- mix(as.mixdata(weir_2008), mixparam(mu=c(14057, 14082, 14107), sigma= c(10,10,10)),constr = mixconstr(consigma="SEQ"), dist= 'gamma', iterlim=5000)) #THis didn't work very well


dist_plot(fit, 2008)
percent_dist(fit, 14069) #date when fifty percent is early run. According to distributions? 
dnormfit(fit, 14069)
percent_dist(fit, 14073)

#2010
weir_2010 <- data_prep(weir_data, 2010)
graph_year(weir_2010)
fit <- distribution_estimation_norms(weir_2010)
(fit <- mix(as.mixdata(weir_2010), mixparam(mu=c(14785, 14800, 14850), sigma= c(10,5,5)),constr = mixconstr(consigma="SEQ"), dist= 'gamma', iterlim=10000)) #THis didn't work
