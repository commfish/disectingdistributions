...
chig_data <-read_csv('../data/ChigISGrunappt2006-2017catch.by.district.csv') %>% 
  ...
  mutate(esc = early_esc_genetics + late_esc_genetics,
         ...
         day_of_year_weir = lubridate::yday(date_weir))%>% # convert the Date to its numeric equivalent
         filter(year(date_weir) %in% c(2006:2008))
...
harvest_data <- read_csv('../data/harvest19692017.csv') %>% 
  ...
  mutate(date_harvest = mdy(date_begin),
         ...
         chig_catch = fish_count*allocation_percent
         )
harv <- harvest_data %>% 
  dplyr::select(date_weir, area, chig_catch) %>% 
  tidyr::spread(area, chig_catch) %>% 
  mutate(catch = rowSums(Central, Chignik, Dolgoi, Eastern, Igvak, Ikatan, Perryville, SEDM, Shumagins, Unimak, Western, na.rm = TRUE))

#Some harvest would have arrived at the weir after the weir is removed
something <-anti_join(harv, chig_data, by ="date_weir")

all <- full_join(chig_data, harv, by = "date_weir") %>%
  mutate(run = rowSums(.[esc, catch] na.rm = TRUE))
