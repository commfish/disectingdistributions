...
chig_data <- read_csv('../data/ChigISGrunappt2006-2017catch.by.district.csv') %>% 
  ...
  mutate(esc = early_esc_genetics + late_esc_genetics,
         ...
         day_of_year = lubridate::yday(date))# -> chig_data # convert the Date to its numeric equivalent

read_csv('../data/harvest19692017.csv') %>% 
  mutate(esc = early_esc_genetics + late_esc_genetics,
         #catch = early_catch_genetics + late_catch_genetics,
         ...
         #run_early_gen = prop_early_genetics*run,
         date_harvest = mdy(date_begin_harvest),
         day_of_year_harvest = yday(date)) -> harvest_data # convert the Date to its numeric equivalent
